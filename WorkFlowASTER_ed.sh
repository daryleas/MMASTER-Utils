...
# Unzipping data and archiving files
mkdir RawData
mkdir zips
nscenes=$(ls *.zip | wc -l)
...
mm3d Malt Ortho ".*${name}(()|_3N|_3B).tif" GRIBin \
    ImMNT="${name}(_3N|_3B).tif" \
    MOri=GRID \
    ZMoy=2500 \
    ZInc=2500 \
    ZoomF=8 \
...
echo "        low-res DEM Max = "$Max
echo "********************************************************"

# Filter obvious error in min/max (limit to earth min/max)
if [ $Min -lt -420 ]; then Min=-420; fi
if [ $Max -gt 8850 ]; then Max=8850; fi
# Next 2 lines is basically if the auto min/max function failed / DEM is really bad, happens if a lot of sea or a lot of clouds
...
if [ "${NoCorDEM}" = true ]; then
    mm3d Malt Ortho ".*${name}(()|_3N|_3B).tif" GRIBin \
        ImMNT="${name}(_3N|_3B).tif" \
        ImOrtho="FalseColor_${name}.tif" \
        MOri=GRID \
        ZInc="${Inc}" \
        ZMoy="${Mean}" \
        ZoomF=1 \
...
mm3d SateLib ApplyParallaxCor "${name}${Bt}" GeoI-Px/Px2_Num16_DeZoom1_Geom-Im.tif \
    FitASTER="${fitVersion}" \
    ExportFitASTER=1 \
    ASTERSceneName="${name}"
...
if [ "${nameWaterMask}" != false ]; then
    WorkFlow_WaterMask.sh "${name}" "${proj}" "${nameWaterMask}"
fi
...
echo "mm3d Malt Ortho \".*${name}(()|_3N|_3B).tif\" GRIBin ImMNT=\"${name}(_3N|_3B).tif\" ImOrtho=\"FalseColor_${name}.tif\" MOri=GRID ZInc=${Inc} ZMoy=${Mean} ZoomF=${ZoomF} ZoomI=32 ResolTerrain=${RESTERR} NbVI=2 EZA=1 DefCor=0 Regul=0.1 ResolOrtho=2 SzW=${SzW}"

# Execute Malt Ortho command
mm3d Malt Ortho ".*${name}(()|_3N|_3B).tif" GRIBin \
    ImMNT="${name}(_3N|_3B).tif" \
    ImOrtho="FalseColor_${name}.tif" \
    MOri=GRID \
...
# Display and execute Tawny command
...
if [[ $(ls Z_Num*_DeZoom*_STD-MALT_Tile_*.tif 2>/dev/null) ]]; then
    
    # Get arrays of final files
    ...
    # Get strings without extensions
    laststr="${lastDEM%.*}"
    Maskstr="${lastMask%.*}"
    corrstr="${lastcor%.*}"

    # Mosaic correlation files if they exist
    if [ -f "${lastcor}" ]; then
        mosaic_micmac_tiles.py -filename "${lastcor:0:${#lastcor}-13}"
    fi
    
    # Mosaic mask files if they exist
    if [ -f "${lastMask}" ]; then
        mosaic_micmac_tiles.py -filename "${lastMask:0:${#lastMask}-13}"
    fi
    
    # Mosaic DEM files if they exist  
    if [ -f "${lastDEM}" ]; then
...
# Make geotif files out of the tif+tfw

# Process correlation file
mv "${lastcor}" Correl_FullRes.tif
cp "${laststr}.tfw" Correl_FullRes.tfw
gdal_translate -tr "${RESTERR}" "${RESTERR}" -a_srs "${proj}" \
    Correl_FullRes.tif "${lastcor}"

# Process mask file
mv "${lastMask}" AutoMask_FullRes.tif
cp "${laststr}.tfw" AutoMask_FullRes.tfw
gdal_translate -tr "${RESTERR}" "${RESTERR}" -a_srs "${proj}" \
    AutoMask_FullRes.tif "${lastMask}"
...
gdal_translate -tr "${RESTERR}" "${RESTERR}" -r cubicspline \
    -a_srs "${proj}" -co TFW=YES \
    Z_FullRes.tif "${lastDEM}"

cd ..

# Generate PLY file if requested
if [ "${do_ply}" = true ]; then
    mm3d Nuage2Ply "MEC-Malt/${lastNuage}" \
        Out="${name}.ply" \
        Attr="Ortho-MEC-Malt/Orthophotomosaic.tif"
fi

# Compute orbit angles on DEM if requested
if [ "${do_angle}" = true ]; then
    
    # Computing orbit angles on DEM for 3N band
    ...
    gdal_translate -a_srs "${proj}" -a_nodata 0 \
        TrackAngleMap_nonGT.tif TrackAngleMap_3N.tif
    rm TrackAngleMap_nonGT*
    
    # Computing orbit angles on DEM for 3B band
    ...
    mv TrackAngleMap.tif TrackAngleMap_nonGT.tif
    gdal_translate -a_srs "${proj}" -a_nodata 0 \
        TrackAngleMap_nonGT.tif TrackAngleMap_3B.tif
    rm TrackAngleMap_nonGT*
fi

# Process orthophotomosaic
cd Ortho-MEC-Malt

# If there are tiles, mosaic them
# Note: assuming tiles will always be Nx1
if [ -f Orthophotomosaic_Tile_0_0.tif ]; then
    mosaic_micmac_tiles.py -filename 'Orthophotomosaic'
fi

# Create full resolution backup and final processed version
mv Orthophotomosaic.tif Orthophotomosaic_FullRes.tif
mv Orthophotomosaic.tfw Orthophotomosaic_FullRes.tfw

gdal_translate -tr 15 15 -r bilinear -a_srs "${proj}" \
    Orthophotomosaic_FullRes.tif Orthophotomosaic.tif
