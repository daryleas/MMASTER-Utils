...
def _argparser():
    ...
    parser.add_argument('-s', '--slavedem', type=str, default=None,
                        help='(optional) name of DEM to correct. By default, mmaster_bias_correction.py looks for MMASTER DEMs of the form AST_L1A_003..._Z.tif')
    parser.add_argument('-a', '--exc_mask', type=str, default=None,
                        help='exclusion mask. Areas inside of this shapefile (i.e., glaciers) will not be used for coregistration [None]')
    parser.add_argument('-b', '--inc_mask', type=str, default=None,
                        help='inclusion mask. Areas outside of this mask (i.e., water) will not be used for coregistration. [None]')
...
def main():
    ...
    if args.nproc > 1 and len(args.indir) > 1:
        if args.nproc > mp.cpu_count():
            print("{} cores specified to use, but I could only find {} cores on this machine, so I'll use those.".format(args.nproc, mp.cpu_count()))
            args.nproc = mp.cpu_count()
        pool = mp.Pool(args.nproc)

        arg_dict = {
            'mst_dem': args.masterdem,
        ...
        for indir in args.indir:
            ...
            if args.slavedem is None:
                ...
                mmaster_bias_removal(args.masterdem,
                                     args.slavedem,
                                     glacmask=args.exc_mask,  # ← CORREGIDO AQUÍ
                                     landmask=args.inc_mask,
                                     out_dir=args.outdir,
                                     pts=args.points,
                                     return_geoimg=False,
